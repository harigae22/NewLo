...
@foreach($images as $key => $image)
    <div x-data="{}" class="w-full lg:p-5 mb-10 lg:w-2/4 flex justify-center ">
        <div class="bg-gray-100 rounded-lg shadow-lg overflow-hidden ">
            <a @click="$dispatch('img-modal', {  imgModalSrc: '{{ asset('storage/images/' . $image->name)  }}' })" class="cursor-pointer">
                <img alt="{{ $image->name }}" class="w-full h-full object-cover object-center" src="{{ asset('storage/images/' . $image->name) }}">
            </a>
        </div>
    </div>
    @if ($key == 0)
