...
<div x-data="inputFormHandler()" class="my-2">
    <template x-for="(field, i) in fields" :key="i">
        <div class="w-full flex my-2">
            <label :for="field.id" class="border border-gray-300 rounded-md p-2 w-full bg-white cursor-pointer">
                ...
                <span x-text="field.file ? field.file.name : '画像を選択'" class="text-gray-600"></span>
            </label>
    ...
    <template x-if="fields.length < 4">
        <button type="button" @click="addField()" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-violet-700 shadow-violet-700/50 hover:bg-violet-800 hover:translate-y-0.5 transform transition">
